	  		...
	  		$('#dg').datagrid({    
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						if(rows.length!=1) {
							alert("Please select and select only one change record!");
							return false;
						}
			    ...
			    columns:[[    
			        ...
			        {field:'bodBuyPrice',title:'进价（元）',width:100,editor: 'numberbox'},  
			        {field:'bodTotalPrice',title:'总价（元）',width:100},   
			        {field:'bodImeiList',title:'串号',width:100,align:'right',editor: 'numberbox'}    
			    ]],  
  ...
  <body>
  	<form class="myfm" style="padding:10px;margin:0px;background:#eee;" id="ff" action="">
	  	...
	  	<div class="myfitem2">
			...
			<label for="shId">仓库类型：</label>
			<select id="cc" class="easyui-combobox" name="shId" >
				<c:forEach items="${applicationScope.sysParam.shId}"
					var="sh">
					<option value="${sh.key}">${sh.key}--${sh.value}</option>
				</c:forEach>
			</select>		
	 		<label>日期：</label><input  type="text" id="boDate" class="easyui-datebox" name="boDate" required="required"/>
	   	</div>
